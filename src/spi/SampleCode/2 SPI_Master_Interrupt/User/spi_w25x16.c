...
/*************<macro>******************/
#define W25X16_DATA_BUF_LEN		(1030)//读取写入BUF长度
...
void W25X16_ReadBytes(uint32_t startAddr, uint8_t *pDataBuf, uint16_t dataLen)
{
	...
	SPI_TransmitReceiveInt(SPI1, g_flashRdDataBuf, g_flashWrDataBuf, (dataLen + 4));//可读取任意长度数据
	while(!(SPI_GetTransmitReceiveStatus(SPI1) & SPI_STATUS_RX_FINISH_MASK));//等待读取完毕
...
void W25X16_WriteBytes(uint32_t startAddr, uint8_t *pDataBuf, uint16_t dataLen)
{
	...
	for (ii = 0; ii < wrTimes; ii++)
	{
		...
		while(!(SPI_GetTransmitReceiveStatus(SPI1) & SPI_STATUS_TX_FINISH_MASK));//等待发送完毕
		SPI_MasterReleaseCS(SPI1);
