...
{
	I2C_MasterTransmitPoll(I2C0, AT24C02_DEV_ADDR, &startAddr, 1, DISABLE);//发送读取地址
	I2C_MasterReceivePoll (I2C0, AT24C02_DEV_ADDR, pDataBuf, dataLen);//读取数据
}
...
void AT24C0X_WriteBytes(uint8_t startAddr, uint8_t *pDataBuf, uint16_t dataLen)
{
	...
	for (ii = 0; ii < wrTimes; ii++)
	{
		...
		I2C_MasterTransmitPoll(I2C0, AT24C02_DEV_ADDR, tempWrBuf, (AT24C02_PAGE_SIZ + 1), ENABLE);
	...
	if (restLen > 0)//非满页,需要单字节方式写入
	{
		for (ii = 0; ii < restLen; ii++)
		{
			...
			I2C_MasterTransmitPoll(I2C0, AT24C02_DEV_ADDR, tempWrBuf, 2, ENABLE);//单次写1个byte
			mdelay(5);//需要等待5ms发送下一条写入命令
		}
	}
}
