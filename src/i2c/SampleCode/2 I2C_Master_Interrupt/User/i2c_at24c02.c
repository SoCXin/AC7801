...
{
	I2C_MasterTransmitInt(I2C0, AT24C02_DEV_ADDR, &startAddr, 1, DISABLE);//发送读取地址
//	while(!(I2C_GetTransmitReceiveStatus(I2C0) & I2C_TRANSMIT_STATUS_FINISHED));
	while(I2C_GetTransmittedReceivedLength(I2C0) < 1);
	I2C_MasterReceiveInt (I2C0, AT24C02_DEV_ADDR, pDataBuf, dataLen);//读取数据
//	while(!(I2C_GetTransmitReceiveStatus(I2C0) & I2C_TRANSMIT_STATUS_FINISHED));
	while(I2C_GetTransmittedReceivedLength(I2C0) < dataLen);
}
...
void AT24C0X_WriteBytes(uint8_t startAddr, uint8_t *pDataBuf, uint16_t dataLen)
{
	...
	for (ii = 0; ii < wrTimes; ii++)
	{
		...
		I2C_MasterTransmitInt(I2C0, AT24C02_DEV_ADDR, tempWrBuf, (AT24C02_PAGE_SIZ + 1), ENABLE);
//		while(!(I2C_GetTransmitReceiveStatus(I2C0) & I2C_TRANSMIT_STATUS_FINISHED));
		while(I2C_GetTransmittedReceivedLength(I2C0) < (AT24C02_PAGE_SIZ + 1));
	...
	if (restLen > 0)//非满页,需要单字节方式写入
	{
		for (ii = 0; ii < restLen; ii++)
		{
			...
			I2C_MasterTransmitInt(I2C0, AT24C02_DEV_ADDR, tempWrBuf, 2, ENABLE);//单次写1个byte
//			while(!(I2C_GetTransmitReceiveStatus(I2C0) & I2C_TRANSMIT_STATUS_FINISHED));
			while(I2C_GetTransmittedReceivedLength(I2C0) < 2);
			mdelay(5);//需要等待5ms发送下一条写入命令
		}
	}
}
